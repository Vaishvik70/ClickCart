...
const SellerNavbar = ({ seller, setSeller }) => {
  ...
  return (
    <nav className="bg-white text-black p-4 flex justify-between items-center shadow-md">
      <h1 className="text-2xl font-bold tracking-wide">Seller Hub</h1>
      ...
      {seller ? (
        <div className="flex items-center gap-6">
          <span className="text-lg font-medium flex items-center gap-2 bg-green-600 text-white px-3 py-1 rounded-lg">
            <FiUser /> {seller.name || seller.email || "Seller"}
          </span>
          <button
            onClick={handleLogout}
            className="bg-red-600 px-4 py-2 rounded-lg font-medium flex items-center gap-2 hover:bg-red-700 text-white transition"
          >
            <FiLogOut /> Logout
          </button>
        </div>
